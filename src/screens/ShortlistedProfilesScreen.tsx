...
const TOTAL_PROFILES = 3;

export const ShortlistedProfilesScreen: React.FC = () => {
  const { theme } = useTheme();
  const [profiles] = useState<Profile[]>(mockProfiles);

  const handleRemoveFromShortlist = useCallback((profileId: string) => {
  ...
  const renderProfileCard = ({ item }: { item: Profile }) => (
    ...
    >
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <Text style={[styles.removeIcon, { color: theme.colors.text }]}>✕</Text>
        <Text style={[styles.removeText, { color: theme.colors.text }]}>
